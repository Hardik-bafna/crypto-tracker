export function compositeForensicWeight(edge, graph) {
...
export function dijkstraShortestPath(graph, sourceAddress, targetAddress, weightFn) {
    ...
    const getWeight = weightFn || ((e) => compositeForensicWeight(e, graph));
    ...
    for (const edge of pathEdges) {
        ...
        if (edge.isCrossChain || edge.bridgeName)
            hasBridge = true;
        const targetNode = graph.getNode(edge.target);
        if (targetNode?.entityType === "MIXER")
            hasMixer = true;
        if (targetNode?.entityType === "BRIDGE")
            hasBridge = true;
    }
