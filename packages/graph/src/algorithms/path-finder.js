 ...
 */
export function findMultipleTracePaths(graph, options) {
    ...
    const isTargetNode = (node) => {
        if (node.address.toLowerCase() === start)
            return false;
        if (destAddr)
            return node.address.toLowerCase() === destAddr;
        if (destEntityType)
            return node.entityType?.toUpperCase() === destEntityType;
    ...
    const foundPaths = [];
    const MAX_PATHS = 5;
    function dfsPathSearch(currentAddr, currentPathNodes, currentPathEdges, visited) {
        if (foundPaths.length >= MAX_PATHS)
            return;
        if (currentPathEdges.length > maxHops)
            return;
        ...
        if (currentPathEdges.length > 0 && isTargetNode(currentNode)) {
            ...
            if (destAddr)
                return; // If specific target found, stop expanding this branch
        }
        if (currentPathEdges.length >= maxHops)
            return;
    ...
    return foundPaths.map((path, index) => {
        ...
        const suspiciousIndicators = new Set();
        for (const node of path.nodes) {
            if (node.entityName)
                knownEntities.add(node.entityName);
            if (node.entityType === "MIXER")
                suspiciousIndicators.add("Privacy Mixer Interaction");
            if (node.entityType === "BRIDGE")
                suspiciousIndicators.add("Cross-Chain Bridge Routing");
            if (node.tags?.includes("ILLICIT"))
                suspiciousIndicators.add("Flagged Illicit Entity");
        }
