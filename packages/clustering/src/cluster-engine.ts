  ...
  clusterWallets(
    ...
    for (const [sender, changeSet] of changeGroups) {
      ...
      if (existing) {
        changeSet.forEach((addr) => {
          if (!existing.members.includes(addr)) existing.members.push(addr);
        });
    ...
    for (const temp of temporalGroups) {
      const existing = clusters.find((c) =>
        temp.addresses.some((a) => c.members.includes(a))
      );
      if (existing) {
        temp.addresses.forEach((a) => {
          if (!existing.members.includes(a)) existing.members.push(a);
        });
