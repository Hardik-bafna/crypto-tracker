...
export function detectTemporalCoActivity(
  transactions: NormalizedTransaction[],
  timeWindowSeconds: number = 300 // 5 minutes
): TemporalCluster[] {
  const addressPairs = new Map<string, { count: number; addrs: [string, string] }>();
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    for (let j = i + 1; j < sorted.length; j++) {
      ...
      if (delta > timeWindowSeconds) break;

      const fromB = txB.from[0]?.toLowerCase();
      if (!fromB || fromA === fromB) continue;
