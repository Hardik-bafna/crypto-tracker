...
export class AIInvestigator {
    ...
    async processQuery(request, investigation) {
        ...
        for (const call of toolCalls) {
            const res = await this.toolDispatcher.executeTool(call.tool, call.parameters, investigation?.graph ? undefined : undefined);
            executionResults[call.tool] = res;
            if (call.tool === "get_evidence" && Array.isArray(res)) {
                citedEvidence.push(...res);
            }
            if (call.tool === "detect_patterns" && res && typeof res === "object" && "evidence" in res) {
                citedEvidence.push(...(res.evidence || []));
            }
        }
...
//# sourceMappingURL=ai-investigator.js.map