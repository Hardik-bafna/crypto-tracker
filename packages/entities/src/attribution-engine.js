export class AttributionEngine {
    ...
    generateAttributions(addresses, transactions, edges) {
        const observedFacts = [];
        const inferences = [];
        const attributions = [];
        ...
        for (const tx of transactions) {
            ...
            observedFacts.push(`[${dt}] Observed transfer of ${amt} on ${tx.chain} from [${fromList}] to [${toList}] (Tx: ${tx.txHash.slice(0, 10)}...)`);
        }
        // 2. Attribute addresses based on intelligence dataset
        for (const addr of addresses) {
            const lower = addr.toLowerCase();
            if (processedAddresses.has(lower))
                continue;
            processedAddresses.add(lower);
            const known = this.entityDb.getEntityByAddress(addr);
            if (known) {
                const { entity, mapping } = known;
                const confidenceScore = mapping.confidence === "VERIFIED"
                    ? 0.99
        ...
        for (const edge of edges) {
            ...
            if (targetKnown && targetKnown.entity.type === "EXCHANGE") {
                inferences.push(`INFERENCE: Address ${edge.source.slice(0, 8)}... transferred ${edge.formattedAmount} directly to ${targetKnown.entity.name} (${edge.target.slice(0, 8)}...). The source address may be an unhosted depositor account for this exchange.`);
            }
            if (targetKnown && targetKnown.entity.type === "MIXER") {
                inferences.push(`INFERENCE: Address ${edge.source.slice(0, 8)}... deposited ${edge.formattedAmount} into privacy pool ${targetKnown.entity.name}. Direct on-chain lineage is obscured beyond this point.`);
            }
            if (sourceKnown && sourceKnown.entity.type === "MIXER") {
                inferences.push(`INFERENCE: Address ${edge.target.slice(0, 8)}... received ${edge.formattedAmount} from privacy pool ${sourceKnown.entity.name}. Likely recipient/relayer withdrawal.`);
            }
        }
        return {
            observedFacts,
...
//# sourceMappingURL=attribution-engine.js.map