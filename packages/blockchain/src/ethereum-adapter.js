...
export class EthereumAdapter extends BaseBlockchainAdapter {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (data && data.hash) {
                        ...
                        const tokenTransfers = (data.token_transfers || []).map((tt) => {
    ...
    async getAddressTransactions(address, options) {
        ...
        if (options?.direction === "inbound") {
            transactions = transactions.filter((tx) => {
                ...
                const tokenInbound = tx.tokenTransfers?.some((t) => t.to.toLowerCase() === lower);
            ...
            transactions = transactions.filter((tx) => {
                ...
                const tokenOutbound = tx.tokenTransfers?.some((t) => t.from.toLowerCase() === lower);
    ...
    synthesizeDynamicEthTxs(address) {
    ...
    async fetchLiveAddressTransactions(address) {
        const liveTxs = [];
        const lower = address.toLowerCase();
        try {
            // 1. Fetch main native transactions up to 3 pages using next_page_params pagination
            let url = `https://eth.blockscout.com/api/v2/addresses/${address}/transactions`;
            let pageCount = 0;
            const MAX_PAGES = 3;
                ...
                if (tokenRes.ok) {
                    ...
                    if (tokenData.items && Array.isArray(tokenData.items)) {
                        for (const item of tokenData.items) {
                            ...
                            if (!txHash)
                                continue;
                            if (liveTxs.some((t) => t.txHash.toLowerCase() === txHash.toLowerCase()))
                                continue;
