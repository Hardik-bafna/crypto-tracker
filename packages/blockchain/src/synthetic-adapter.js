...
export const SYNTHETIC_DEMO_CASES = [
    {
        id: "case-silk-trail",
        name: "Operation Silk Trail (Narcotics Pipeline)",
        category: "Narcotics Trafficking & Mixer Evasion",
        description: "Tracing illicit drug proceeds moving from a flagged Darknet vendor wallet through a peel chain, mixing pool (Tornado Cash), cross-chain bridge, and ending in a KYC Exchange deposit.",
        ...
        narrative: "Investigative target 0x98174f85e49f87f4c9c1b3f9429188d3f6a2b001 is linked to fentanyl distribution marketplaces. On 2026-08-01, 150 ETH was moved through intermediary addresses, peeled into smaller increments, anonymized via Tornado Cash, routed across Synapse Bridge, and finally deposited to Binance.",
    },
    {
        id: "case-hydra-flow",
        name: "Operation Hydra Flow (Syndicate Fan-Out)",
        category: "Money Laundering & Fan-Out Structuring",
        description: "Suspected cartel distribution hub executing a 1-to-8 fan-out dispersal to smurfing mules, followed by rapid multi-wallet hops and fan-in aggregation.",
        ...
        narrative: "A primary distribution node bc1q9d8e7... dispersed 24.5 BTC across 8 unhosted wallets within 15 minutes, which subsequently hopped through secondary addresses before reconverging at a centralized deposit address.",
    },
    {
        id: "case-phantom-vault",
        name: "Operation Phantom Vault (USDT Token Laundering)",
        category: "Stablecoin Laundering & Structuring",
        description: "High-hop layered structuring of $500,000 USDT across multiple synthetic intermediate addresses and swap protocols.",
        ...
        expectedRiskLevel: "HIGH",
        narrative: "High-velocity ERC-20 Tether transfers structuring large payments across layered intermediary addresses to avoid exchange AML thresholds.",
    },
];
export class SyntheticBlockchainAdapter extends BaseBlockchainAdapter {
    ...
    async getAddressTransactions(address, options) {
        ...
        if (options?.direction === "inbound") {
            list = list.filter((tx) => tx.to.some((t) => t.toLowerCase() === lower) ||
                tx.tokenTransfers?.some((tt) => tt.to.toLowerCase() === lower));
        }
        else if (options?.direction === "outbound") {
            list = list.filter((tx) => tx.from.some((f) => f.toLowerCase() === lower) ||
    ...
    synthesizeDynamicCaseForAddress(address) {
...
//# sourceMappingURL=synthetic-adapter.js.map