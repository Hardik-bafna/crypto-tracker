export class RiskEngine {
    ...
    evaluate(params) {
        ...
        if (mixerPatterns.length > 0) {
            ...
            recommendations.push("Request withdrawal relayer metadata and deposit timing correlations for identified mixer pools.");
        }
        ...
        if (illicitPatterns.length > 0) {
            ...
            recommendations.push("Issue formal 18 U.S.C. preservation request and prepare search warrant for linked accounts.");
        }
        ...
        if (peelPatterns.length > 0) {
            ...
            recommendations.push("Monitor peeled change outputs for eventual consolidation or unhosted ATM cashouts.");
        }
        ...
        if (bridgePatterns.length > 0) {
            ...
            recommendations.push("Serve 2703(d) order or MLAT request to destination chain bridge validator operators.");
        }
        ...
        for (const factor of factors) {
            ...
            for (const evId of factor.evidenceIds) {
                ...
                const parentPattern = params.patterns.find((p) => p.evidence.some((pe) => pe.id === evId));
                // Try to resolve entity info from nodes involved
                const involvedAddresses = ev?.addresses ?? [];
                const matchedNode = nodes.find((n) => n.entityName &&
                    involvedAddresses.some((a) => a.toLowerCase() === n.address.toLowerCase()));
                chain.push({
                    evidenceId: evId,
    ...
    evaluateConfidence(params) {
        ...
        if (params.patterns.length > 0) {
            const avgPatternConfidence = params.patterns.reduce((sum, p) => sum + p.confidence, 0) / params.patterns.length;
        ...
        if (exchangeNodes.length > 0) {
            strengths.push(`Cash-out endpoint identified at ${exchangeNodes.map((n) => n.entityName || "an exchange").join(", ")}`);
        }
        else {
